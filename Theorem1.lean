...
/-!
# Theorem 1: the `r = 3` counterexample
...
* the hand-certified `r = 3` assembly of `lib/Assembly3.lean`, which gives the
  weaker gain `1/4000` (`threeDegenerateExtremalCounterexample_exact`);
* the general-`r` pipeline of `lib/AssemblyR.lean` specialized at `r = 3`,
  fed with Lemma C's window bound for `r = 3`, which gives the sharp gain
  `1/160` (`threeDegenerateExtremalCounterexample_sharp`).

The second is the headline. The first is kept because it is genuinely
independent of the general-`r` machinery: `lib/Assembly3.lean` does not import
`lib/AssemblyR.lean`.
-/
