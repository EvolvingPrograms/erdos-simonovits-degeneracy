...
/-!
# Theorem 1.3(b): the exponent family law
...
(`DegeneracyLawB.epsMaxR_anti`, `proofs/lib/LedgerAsym.lean`) is the ceiling of the
method and is not attained.

The tuned schedule and its analytic input are `proofs/lib/LedgerTuned.lean`; the
`r(1 - β_r)` asymptotics that close the argument are `proofs/lib/LedgerAsym.lean`.
-/
