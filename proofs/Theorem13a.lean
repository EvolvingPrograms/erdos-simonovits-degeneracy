...
/-!
# Theorem 1.3(a): the limit of the rescaled feasibility window
...
`proofs/lib/WindowLimit.lean`; this file states the theorem and discharges the
center-maximization hypothesis on the full subcritical range using the
quantified Lemma 4.2 (`DegeneracyLawQuant.supG_eq_center_quant`).

Sharpness — the rescaled width tends to `-∞` once `λ ln 2 > 1`, so the phase
transition sits exactly at Gibbs weight `2^λ = e` — is `WindowSharp.lean`.
-/
...
/-- **Theorem 1.3(a), unconditional, full subcritical range.** For every fixed
`λ` with `λ ln 2 < 1`, `r² · width_r → W(λ) = λ⁴ ln³2 / 64`.  The sandwich
only needs Lemma 4.2 eventually in `r`, so the quantified Lemma 4.2
