...
This file closes the one analytic gap left open in `proofs/lib/LedgerTuned.lean`: the limit

  `r (1 - β_r) → 1/(8 ln 2)`

which `sixteen_rsq_epsMax_tendsto` took as an explicit hypothesis.  With it,
Theorem 1.3(b) — `16 r² ε^max_r → 1` — becomes **unconditional**.
...
/-! ## §5 Theorem 1.3(b), unconditional -/

/-- **Theorem 1.3(b) (the design notes Thm 10.6(b)), unconditional.**
