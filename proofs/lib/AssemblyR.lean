...
/-!
# The general `r` assembly

Machinery for Theorem 1.2: for every `r ≥ 2` a connected bipartite `r`-degenerate
graph `H` and constants `c, ε > 0` with `c · n^(2 − 1/r + ε) ≤ ex(n, H)`
eventually.  The target statements built on top of this file are
`Theorem12.lean` (every `r`, gain `1/(28 r²)`) and the sharp `r = 3` form in
`Theorem12r3.lean` (gain `1/160`).

This is the `r`-generic analogue of `proofs/scratch/Assembly3.lean` (the `r = 3` assembly).
...
* **Block E** — the numeric parameters, generic in `r`, taken from `LedgerR`
  ...
  of `proofs/lib/WindowLimit.lean`.
* **Block F** — the `SimpleGraph.Copy` transport (`proofs/scratch/Assembly3.lean` 1018–1555),
  phrased over the generic counterparts, on top of `BridgeR` §7.
-/

namespace DegeneracyLaw

/-- **Theorem 1.2's analytic engine, unconditional.** Window positivity at
`λ = 27/20` for every `r ≥ 2`: Lemma 4.2 (`LemmaB.supG_eq_center`) discharges the
`hB` hypothesis of `Theorem13a.width_pos_at_135'`. -/
...
exclusion edge — see `proofs/lib/LedgerSharp.lean` and Theorem 1.3(b)'s family law):
...
unconditional) plus the *upper* window bound (`Theorem13a`). -/
...
/-- **The window upper bound**, from `WindowUpper`. -/
...
`(q, v) ∈ [0,1]²` and the image is bounded above (`WindowUpper`'s
...
`Theorem13a.lowerSeq_le_rsq_width` (Lemma 4.4, discharged by `LemmaB`) gives
...
/-- Lemma 4.4's window bound specialized at `r = 3`: the generic `lowerSeq_ge`
...
/-- **The sharp `r = 3` exponent gain**: `ε₃ ≥ 1/160`. -/
...
optimized `(θ, η) = (1/100, 1/100)` ledger of `proofs/lib/LedgerSharp.lean`,
...
/-- **Theorem 1.2.**  For every `r ≥ 2` there is a connected bipartite graph
...
/-- **Theorem 1.2 with the explicit exponent** `ε = 1/(28 r²)`, conditional on the
Gibbs bound. -/
...
/-- **Theorem 1.2 with the explicit exponent**, unconditional. -/
...
/-- **Theorem 1.2**, unconditional: the Gibbs variational bound
...
midpoint evaluation of the same ledger, roughly `1/590` (and far past
the hand-certified `1/4000` of `proofs/scratch/Assembly3.lean`).  The
generic pipeline at `r = 3`, with Lemma 4.4's window bound specialized at
