...
Machinery for Theorem 1.3(a); the headline limit itself is
`DegeneracyLaw.width_tendsto_unconditional_full` in `Theorem13a.lean`.

The two-sided bounds are already available:

* `DegeneracyLaw.width_le_mul` (the window upper bound, **unconditional**) gives
  `width r λ ≤ W(λ)/r² · (1 - 1/r)`;
* `DegeneracyLaw.width_ge` (Lemma 4.4, conditional on Lemma 4.2) gives
...
Lemma 4.2 is taken here as a hypothesis
...
/-- The rescaled upper bound sequence `W(λ)(1 - 1/r)` from the window upper bound. -/
def upperSeq (lam : ℝ) (r : ℕ) : ℝ := Wconst lam * (1 - 1 / (r : ℝ))

/-- The rescaled lower bound sequence from Lemma 4.4. -/
...
/-- **Theorem 1.3(a) (the design notes §6.7-6.8).** Conditional on Lemma 4.2 (`hB`), the rescaled
...
/-- **Eventual positivity at fixed `λ`.** From Theorem 1.3(a) and `W(λ) > 0`. -/
...
/-! ## Unconditional form: `hB` discharged by Lemma 4.2 -/

/-- **Theorem 1.3(a), unconditional** for subcritical `λ ≤ 27/20`: Lemma 4.2
