...
/-!
# The window upper bound `width_r ≤ W(λ)/r² (1 - 1/r)`

The unconditional half of the Theorem 1.3(a) sandwich: no choice of parameters
...
the entropy-series bound -- are `proofs/lib/WindowUpperBound.lean`.
-/
...
/-- **The window upper bound** (the design notes §6.6). For `r ≥ 2`, `λ > 0` and `λ ln 2 < 2r`
...
theorem width_le_mul (r : ℕ) (lam : ℝ) (hr : 2 ≤ r) (hlam : 0 < lam)
  ...
  have hC : Cside r (tauOf r lam) ≤
    ...
    linarith
  -- A-side (Steps 1 & 3): the center is a competitor, and log cosh is bounded below
...
/-- **The window upper bound, plain form.** `width_r ≤ W(λ)/r²`. -/
