...
/-!
# Window upper bound machinery: the center value and the entropy defect

Machinery for the window upper bound; the bound itself is `WindowUpper.lean`.

Contains the global `log cosh` lower bound of §5.1, the exact binomial moments
of §5.2, the entropy-defect lower bound of §5.3, the exact value of `G_r` at
the center, and `bddAbove_image2_Gfun` -- which is what stops `supG` from
being a junk `sSup`.
-/
...
The two-sided log cosh toolkit (`sinh`/`tanh` ladder, `log_cosh_le`,
`le_log_cosh`) has a single home in `Lemma44.lean`.  We only re-export the lower
half here under the name this file and its downstream users refer to. -/

/-- **The two-sided log cosh bound (lower half).** `t²/2 - t⁴/12 ≤ log cosh t` for every real `t`.
...
`Lemma44.lean` proves these once, for the unnormalised moment
...
/-! ## Step 2 of §6.6: the exact value of `G_r` at the center -/
...
/-! ## Step 1 of §6.6: the center is a competitor for the supremum -/
...
/-- **Step 1.** The center value is at most the supremum. -/
...
/-! ## Step 4: assembly — the window upper bound -/
