...
/-!
# Lemma 4.2 machinery: the Bernstein operator, the angular strip, and the
# off-strip certificate

Machinery for Lemma 4.2; the lemma itself -- `sup G_r = G_r(1/2,1/2)` -- is
`Lemma42.lean`.

Contains the `θ`-form of `G_r`, the three sharp derivative bounds of §4.2, the
Bernstein operator and its curvature transfer (§4.3), the in-strip concavity
argument (§4.4a) and the off-strip numeric certificate (§4.4c).
-/
...
The exact center value `G_r(1/2,1/2) = centerSum r lam` (paper (4.13)) is proved
once and for all in `WindowUpper.lean` as `DegeneracyLaw.Gfun_center_eq`; this file
...
/-- **Even + concave ⇒ the center dominates.** -/
...
1-dimensional one by restricting `G_r` to the segment joining the center
...
/-- `G_r` restricted to the segment through the center in direction
...
segment through the center staying inside the strip is concave. See the section
...
/-- **Prop. 4.1 + (4.10)**: inside the strip the center dominates. -/
...
`g₀(q,θ) = F_a(q)/2 + log₂ cos(θ - θ*(q))`, Lemma 4.1's quadratic margin
...
There is a "rotation center" `ts = arctan e^{a(2q-1)}` with
`|ts - π/4| ≤ a|q - ½|` such that

`G_r(q,v) ≤ 1 - λ/2 - ((1-a²)(q-½)² + (θ-ts)²/2)/ln2 + a²/(8r ln2)`,

where `θ = thetaOf v`.  This is Lemma 4.1's margin (`Ffun_le_center`) plus the
...
differ only in how the quadratic margin is compared with the center defect. -/
...
theorem Gfun_le_center_offstrip (r : ℕ) (lam q v : ℝ) (hr : 2 ≤ r)
  ...
  have hcert : a ^ 4 / 256
    ...
    exact h
  -- Step 6: the center value from below
  have hcen : 1 - lam / 2
