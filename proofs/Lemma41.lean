...
/-!
# Lemma 4.1: far field / concavity of `F_a`
...
* `Ffun_second_deriv` : the exact second-derivative identity
  ...
  which is the source of the phase transition in Theorem 1.3(a).
...
The derivative computations behind these are `proofs/lib/EntropyConcavity.lean`.
-/
...
/-- **Lemma 4.1 (derivative identity).** For `q ∈ (0,1)`, the function `ln 2 · F_a` has first
...
/-- **Lemma 4.1 (downstream form).** For `0 < a < 1` and `q ∈ [0,1]`,
