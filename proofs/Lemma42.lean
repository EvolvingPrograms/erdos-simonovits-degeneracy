...
/-!
# Lemma 4.2: the supremum of `G_r` sits at the symmetric point
...
through a Bernstein operator in `q`, and Lemma 4.1 away from a central strip;
all of that development is `proofs/lib/BernsteinStrip.lean`.

A quantified version valid for every subcritical `λ` once `r ≥ R(λ)` is
`proofs/lib/CenterMaxThreshold.lean`; it is that version which makes Theorem 1.3(a)
...
/-- **The pointwise form of Lemma 4.2**: the center dominates on the whole box. -/
...
/-- **Lemma 4.2** (paper §4) at the Theorem-1.2 operating point `λ ≤ 1.35`,
