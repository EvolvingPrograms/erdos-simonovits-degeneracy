...
/-!
# Lemma C: the window lower bound (paper §5)

This file develops, bottom-up:

* §1 A one-sided mean-value helper.
* §2 The two-sided `log cosh` bounds of Lemma 5.1 (proved *globally* on `ℝ`,
...
* §5 The assembly (5.6) and the corollary at `λ = 27/20`.
-/
...
/-! ## §2 Two-sided `log cosh` bounds (paper Lemma 5.1) -/
...
/-- **Paper Lemma 5.1, upper branch** — valid on all of `ℝ`. -/
...
/-- **Paper Lemma 5.1, lower branch** — valid on all of `ℝ`. -/
...
`Mom p r = ∑_j C(r,j) (2j-r)^p`; the normalised statement divides by `2^r`. -/
...
/-- Normalised sixth moment, the form used in the paper. -/
...
/-- **Lemma C, bound (5.6)**: the window lower bound. -/
