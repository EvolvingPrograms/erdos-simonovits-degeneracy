...
/-!
# Lemma 6.1 machinery: the centre value and the entropy defect

Machinery for Lemma 6.1; the window upper bound itself is `Lemma61.lean`.
...
the centre, and `bddAbove_image2_Gfun` -- which is what stops `supG` from
being a junk `sSup`.
-/
