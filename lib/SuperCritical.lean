...
Machinery for Proposition 6.3; the sharpness statement itself is
`DegeneracyLawSuper.threshold_sharp` in `Prop63.lean`.
...
* **(iii) Above threshold the centre is a strict local minimum of `F_a`.**  The
  Lemma A identity `F_a''(q) ln2 = -1/(q(1-q)) + 4a² sech²(a(1-2q))` is an
...
Together with Theorem 3(a) (`W(λ) = λ⁴ln³2/64 > 0` for `λ < 1/ln 2`) and
Theorem 3(b) (the endpoint `λ*` is attained along the drifting schedule), this
...
/-- **Prop. 6.3, step (iii).**  Above threshold (`a > 1`) the centre is a strict
local *minimum* of `F_a`: for any `t` with `t² ≤ ⅛` and
`16t²(2+a⁴) ≤ 2(a²-1)`,

  `F_a(½ + t) ≥ F_a(½) + (a²-1)t²/ln 2`.

The Lemma A identity `F_a'' ln2 = -1/(q(1-q)) + 4a²sech²(a(1-2q))` is used with
the *opposite* sign to Lemma A's own use of it. -/
...
open LemmaB in
/-- **Prop. 6.3, step (i): Jensen removes `r`.**  The Bernstein operator is an
...
open LemmaB in
/-- **Prop. 6.3, steps (i)+(ii).**  For every `r ≥ 1` and `q ∈ [0,1]` there is a
...
/-- **Prop. 6.3, (6.10).**  For `a = λ ln 2 > 1`, `r ≥ 1` with `a ≤ r`, and any
...
/-- **Proposition 6.3, (6.10), explicit.**  For `λ > 1/ln 2` and every `r ≥ 1`
with `λ ln 2 ≤ r`,
...
/-- **Prop. 6.3, the consequence.**  For `λ > 1/ln 2`,

  `r² · width_r ⟶ -∞`,   i.e.  `W(λ) = -∞`.

The centre loses by the *constant* `δ(a) > 0`, so the window closes at rate
