...
Machinery for Theorem 2: for every `r ≥ 2` a connected bipartite `r`-degenerate
graph `H` and constants `c, ε > 0` with `c · n^(2 − 1/r + ε) ≤ ex(n, H)`
eventually.  The target statements built on top of this file are
`Theorem2.lean` (every `r`, gain `1/(28 r²)`) and the sharp `r = 3` form in
`Theorem1.lean` (gain `1/160`).

This is the `r`-generic analogue of `lib/Assembly3.lean` (the `r = 3` assembly).
...
* **Block E** — the numeric parameters, generic in `r`, taken from `LedgerR`
  ...
  of `lib/WindowLimit.lean`.
* **Block F** — the `SimpleGraph.Copy` transport (`lib/Assembly3.lean` 1018–1555),
