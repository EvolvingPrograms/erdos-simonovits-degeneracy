...
/-!
# The tuned sequence behind Theorem 3(b)

Machinery for Theorem 3(b); the headline family law itself is
`DegeneracyLawB.eight_rsq_epsMax_theta_tendsto` in `Theorem3b.lean`.
...
limit `λ ↑ λ* = 1/ln 2`.  No *fixed* `λ` reaches `λ*`, because Lemma B's
threshold `R(a)` blows up like `(1-a²)⁻¹` (Cor. 10.2, `DegeneracyLawQuant.Rreal`).

Theorem 3(b) escapes by letting `λ` drift:
...
| **Lemma B along the tuned sequence** | `supG_eq_center_tuned` | ✅ |
...
outruns Lemma B's own threshold. -/
...
/-- The Lemma C lower bound at the drifting `λ`, i.e. paper (10.1). -/
...
so `16 r² ε^max_r → 1`, and Theorem 2's fixed slack `η_r = ½` (which would give
