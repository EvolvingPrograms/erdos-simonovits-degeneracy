...
possible.  The qualitative Lemma B lives in `LemmaB.lean`, which is now `sorry`-free; this
...
2. **Lemma 10.1** — `DeltaLB_le_deficit` : `Δ_min(a) ≥ u ρ² / (ln2 (a²+2u))`,
   ...
   * `quad_min_key` : the exact quadratic-minimisation identity
...
4. **The two analytic inputs** — the Bernstein/centre gap (4.12)–(4.14) and the
   strip bound (4.10)
...
/-! ## 2b. The quadratic minimisation (paper Lemma 10.1) -/

/-- The exact identity behind the quadratic minimisation of Lemma 10.1:
...
/-- The quadratic minimisation of Lemma 10.1, in raw algebraic form:
...
/-- `R_of` always demands `r ≥ 2`, as Lemma B's Bernstein step needs. -/
lemma two_le_R_of (lam : ℝ) : 2 ≤ R_of lam := le_max_left _ _

/-! ## 3'. Paper Lemma 5.1: `log cosh t ≥ t²/2 - t⁴/12`

This is the single-variable ingredient of the Bernstein/centre gap (4.14).
...
(`Lemma61.lean`, an alias for the canonical `DegeneracyLaw.LemmaC.le_log_cosh`)
...
`LemmaB.lean` (now sorry-free).  This section instantiates that machinery with
...
unperturbed centre value by the full deficit, up to the Bernstein excess
`a²/(8 r ln 2)`.
...
/-- **(4.13) + (4.14), quantified.**  The centre value exceeds
...
`DegeneracyLaw.centerSum_lower` (Lemma 6.1's Taylor bound, which already carries
...
fixed point is the centre, so the centre dominates.  No `r₀` is needed here. -/
...
/-! ## 5. Assembly (Lemma B with the explicit threshold `R_of`) -/

/-- **Lemma B, quantified, pointwise form.**  For every subcritical `λ` and
every `r ≥ R_of λ`, the centre dominates `G_r` on `[0,1]²`. -/
...
/-- **Lemma B with the explicit threshold (Prop. 10.3's engine).**
...
Theorem 3(b) available at every `r ≥ 2` with no unproved finite checks. -/
