...
* `not_isDegenerate_of_witness` — a finite set in which every vertex has
  more than `k` neighbours refutes `k`-degeneracy;
...
/-- A finite set in which every vertex has more than `k` neighbours refutes
`k`-degeneracy. -/
...
theorem not_isDegenerate_of_layer_witness [Fintype L] [DecidableEq L]
  ...
  · rintro v hv
    ...
    · -- `v = e0 a` is a root: the children `insert a T` for
      ...
      -- `C(card L - 1, r-1) ≥ C(r, r-1) = r` neighbours within `s`
      ...
      omega
    · -- `v = e1 c` is a child: its `r` roots are neighbours within `s`
