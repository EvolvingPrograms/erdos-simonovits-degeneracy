...
Generalises `Bridge3.lean` from `r = 3` to arbitrary `r`.  The `2^r` outcome
...
* §2 balancedness of the `r`-subset design (`Kernel3` 484–579, generalised:
...
*uniform* on `Fin L`.  Generalisation of `Kernel3` lines 484–579
...
in `results_Z_lean_bridgeR.md`) can consume it without importing `Theorem1`. -/
...
`Theorem1.potential_increment_three`.  `error` is the without-replacement
...
The `r`-generic `Theorem1.le_choose_three_of_four`. -/
